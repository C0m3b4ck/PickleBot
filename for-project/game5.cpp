...
const int eval_map[6] = {1000, -1000, 1000, -1000, 500, 500};
...
int eval_board(const char* temp, int x, int y)
{
    // Initialize score to 0
    int score = 0;

    // Check if move is valid (for example, not moving off the edge of the board)
    if (x < 0 || x > 7 || y < 0 || y > 7)
    {
        return -100; // Invalid move
    }

    // Assign a value to each piece on the board based on its type
    static char piece;
    switch (temp[x * 8 + y])
    {
        case 'R':
    ...
    if (piece == 'w' || piece == 'W')
    {
        // If the move is for a white piece, add its value to the score
        switch (y)
        {
            case 0:
                result = eval_map['P'];
    ...
    {
        // If the move is for a black bishop, subtract its value from the score
        switch (y)
        {
            case 0:
                result = -eval_map['P'];
        ...
        // If the move is for a white queen, add its value to the score
        switch (y)
        {
            case 0:
                result = eval_map['P'];
    ...
    {
            // If the move is for a white king, add its value to the score
            switch (y)
            {
                case 0:
                    result = eval_map['P'];
...
pair<int,int> predict_best_move(bool whiteSide) {
    ...
    if (moves.empty()) {  // Check for empty list
    ...
    for (auto mv : moves) {
        ...
        int score = eval_board(temp, mv.first, mv.second);

        // Adjust the score based on the type of piece.
        char piece = (mv.first > 0 ? 'w' : 'b');  // white or black
        switch (piece) {
            case 'w':
                switch (mv.second) {
                    case 0:   // White pawns have a higher value than blacks pawns
                        score += eval_map[1]; // Changed to access the second element of the array
                        break;
                    default:
                        score -= eval_map[piece-88]; // Adjusted the index to match 'w' or 'b'
                        break;
                }
                break;
            case 'b':
                switch (mv.second) {
                    case 0:   // Black pawns have a lower value than white pawns
                        score += -eval_map[5]; // Changed to access the last element of the array
                        break;
                    default:
                        score -= eval_map[piece-88]; // Adjusted the index to match 'w' or 'b'
                        break;
