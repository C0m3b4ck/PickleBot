...
// Global brush for light/dark squares
HBRUSH hBrushLight = NULL;
HBRUSH hBrushDark = NULL;

HBRUSH CALLBACK WndCtlColorBtn(HWND hwndDC, HDC hdc, HWND hwndChild, UINT CtrlType) {
    if (CtrlType != CTLCOLOR_BTN) return NULL;

    // Get square index from GWLP_USERDATA
    LONG_PTR idx = GetWindowLongPtr(hwndChild, GWLP_USERDATA);
    if (idx < 0 || idx >= 64) return NULL;

    bool dark = isSquareDark((int)idx);
    HBRUSH br = dark ? hBrushDark : hBrushLight;

    // Set text color based on piece
    char p = board[(int)idx];
    if (!isEmptySquare(p)) {
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
        case WM_CTLCOLORBTN: {
            ...
            return (LRESULT)WndCtlColorBtn(hwnd, hdc, hwndChild, CTLCOLOR_BTN);
        }
        case WM_CREATE: {
            initialize_game_state();

            hBrushLight = CreateSolidBrush(RGB(235, 210, 160)); // light beige
            hBrushDark  = CreateSolidBrush(RGB(100, 140, 200)); // dark blue-ish
            ...
            for (int rank = 7; rank >= 0; --rank) {
                for (int file = 0; file < 8; ++file) {
                    ...
                    // Store square index in GWLP_USERDATA
                    SetWindowLongPtr(hSquareBtn[idx], GWLP_USERDATA, (LONG_PTR)from);
                    idx++;
                }
            }
            ...
            if (wmId == IDOK || hStartBtn == (HWND)lParam) {
                ...
                for (int i = 0; i < 64; i++) {
                    if (hSquareBtn[i] == (HWND)lParam) {
                        // Retrieve square index from GWLP_USERDATA
                        LONG_PTR sqIdx = GetWindowLongPtr(hSquareBtn[i], GWLP_USERDATA);
                        OnSquareClicked((int)sqIdx);
                        break;
                    }
                }
            }
