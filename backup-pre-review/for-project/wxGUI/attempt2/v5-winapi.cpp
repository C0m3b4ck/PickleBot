...
// For colored squares: track square index per button
int btnSquareIdx[64];
...
int score_move_profile(const char b[64], int from, int to, BotProfile profile, bool moverWhite, MoveInfo& info) {
    char moving = b[from], target = b[to];
    info.from = from;
    info.to = to;
    info.score = 0;
    info.isCapture = !isEmptySquare(target) && isOppositeColor(moving, target);
    info.givesCheck = false;
    info.isKingThreat = false;
    ...
    if (temp[to] == 'P' && to / 8 == 7) temp[to] = 'Q';
    if (temp[to] == 'p' && to / 8 == 0) temp[to] = 'p';
    info.givesCheck = is_in_check(!moverWhite, temp);
    int checkBonus = info.givesCheck ? 40 : 0;
    int kingThreat = 0;
    if (profile == STRATEGIC) {
        kingThreat += king_distance_score(to, !moverWhite);
        if (checkBonus) kingThreat += 60;
        info.isKingThreat = kingThreat > 50;
    }
    int tradePenalty = 0;
    if (profile == SAFE && info.isCapture) {
...
vector<MoveInfo> generate_moves_with_info(bool whiteSide) {
    ...
    for (int from = 0; from < 64; from++) {
        ...
        for (int to = 0; to < 64; to++) {
            if (!is_legal_move(from, to, whiteSide)) continue;
            MoveInfo info;
            score_move_profile(board, from, to, botProfile, whiteSide, info);
...
MoveInfo predict_best_move_info(bool whiteSide) {
    vector<MoveInfo> moves = generate_moves_with_info(whiteSide);
    MoveInfo best;
    best.from = -1;
    best.to = -1;
    best.score = INT_MIN;
    best.isCapture = false;
    best.givesCheck = false;
    best.isKingThreat = false;
...
// Custom paint for buttons to draw colored squares + piece labels
LRESULT CALLBACK SquareProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, UINT_PTR idSubclass, DWORD_PTR dwRefData) {
    ...
    switch (msg) {
        case WM_PAINT: {
            ...
            bool dark = isSquareDark(idx);
            HBRUSH brush;
            COLORREF bg;
            if (dark) {
                bg = RGB(100, 140, 200); // medium blue-ish dark
                brush = CreateSolidBrush(bg);
            } else {
                bg = RGB(235, 210, 160); // light beige
                brush = CreateSolidBrush(bg);
            }
            ...
            char p = board[idx];
            if (p != ' ') {
                bool whiteP = isWhitePiece(p);
                SetBkMode(hdc, TRANSPARENT);
                // Text color: dark on light squares, light on dark squares
                SetTextColor(hdc, dark ? RGB(255,255,255) : RGB(0,0,0));
                // Slight piece-color tint: if piece is white, make it a bit lighter; if black, darker
                if (!whiteP && !dark) {
                    // black piece on light square: draw black
                    SetTextColor(hdc, RGB(0,0,0));
                } else if (whiteP && dark) {
                    // white piece on dark square: draw white
                    SetTextColor(hdc, RGB(255,255,255));
                }

                SetTextColor(hdc, whiteP ? RGB(255,255,255) : RGB(0,0,0));
                // Draw piece char centered
                char buf[2] = { p, 0 };
                RECT r = rc;
...
void RefreshBoard() {
    ...
    for (int i = 0; i < 64; i++) {
        ...
        // We don't set text anymore; painting uses board[i] directly
        // Still set an empty label to avoid default text if any
        SetWindowTextA(hSquareBtn[i], "");
    }
    char s1[128];
...
void MakeBotMove() {
    ...
    MoveInfo mv = predict_best_move_info(botWhite);
    if (mv.from == -1) {
    ...
    const char* profName = (botProfile == OFFENSIVE) ? "Offensive" : (botProfile == SAFE ? "Safe" : "Strategic");
    char logBuf[512];
    int moveCount = 0;
    {
        vector<MoveInfo> tmp = generate_moves_with_info(botWhite);
        moveCount = (int)tmp.size();
    }
    ...
    sprintf(logBuf,
        ...
        "Profile: %s\r\n"
        "Moves considered: %d\r\n"
        "Best move: %s -> %s (score: %d)\r\n"
        "  Capture: %s\r\n"
        "  Gives check: %s\r\n"
        "  King threat: %s",
        index_to_algebraic(mv.from).c_str(),
        index_to_algebraic(mv.to).c_str(),
        mv.score,
        mv.isCapture ? "Yes" : "No",
        mv.givesCheck ? "Yes" : "No",
        mv.isKingThreat ? "Yes" : "No");

    AppendLog(logBuf);

    apply_move(mv.from, mv.to, botWhite);
        ...
        case WM_CREATE: {
            ...
            for (int rank = 7; rank >= 0; --rank) {
                for (int file = 0; file < 8; ++file) {
                    ...
                    hSquareBtn[idx] = CreateWindowA("BUTTON", "",
                        WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
                        boardX + file * (btnSize + gap),
                        boardY + (7 - rank) * (btnSize + gap),
                        btnSize, btnSize, hwnd, NULL, NULL, NULL);
                    btnSquareIdx[idx] = from;
                    // Subclass button to draw colored squares
                    SetWindowSubclass(hSquareBtn[idx], SquareProc, idx, (DWORD_PTR)from);
                    idx++;
                }
            }
            ...
            if (wmId == IDOK || hStartBtn == (HWND)lParam) {
                ...
                for (int i = 0; i < 64; i++) {
                    if (hSquareBtn[i] == (HWND)lParam) {
                        OnSquareClicked(btnSquareIdx[i]);
                        break;
                    }
                }
            }
